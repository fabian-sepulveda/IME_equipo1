...
dif_obs_quick_bubble <- obtiene_media_difs(datos_anchos, "Quicksort", "Bubblesort")
dif_obs_quick_merge <- obtiene_media_difs(datos_anchos, "Quicksort", "Mergesort")
dif_obs_bubble_merge <- obtiene_media_difs(datos_anchos, "Bubblesort", "Mergesort")

# Obtiene las distribuciones de las medias de las diferencias permutadas
dist_medias_difs_quick_bubble <- sapply(permutaciones, obtiene_media_difs, "Quicksort", "Bubblesort")
dist_medias_difs_quick_merge <- sapply(permutaciones, obtiene_media_difs, "Quicksort", "Mergesort")
dist_medias_difs_bubble_merge <- sapply(permutaciones, obtiene_media_difs, "Bubblesort", "Mergesort")

# Obtener valores p.
# num <- sum(abs(dif_quick_bubble) > (abs(dif_obs_quick_bubble) + 1)) # Ojo que tu script tenía un error
